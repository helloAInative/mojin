...
- 总览、自选分组与一键扫描、模拟账户、持仓估值、风险中心、信号、后验、AI 仲裁和 WebSocket 日志
- 当前自选分组可直接运行五路 AI 候选排名，单次最多分析 12 只
- 成交明细与按股票汇总、指数池启停维护
- 每日复盘聚合昨日美股、行业、账户、成交、信号和后验，可复制或下载 Markdown
- 候选股展示波动率、ATR、止损止盈和仓位计划，并可带入模拟下单
- 模拟买单保存风险计划，风险中心按最新行情提示止损、止盈并可带入人工卖单
- 历史回测支持持有期、样本范围与往返成本配置，展示权益曲线和逐笔样本
- 新闻、昨日美股、行业板块、历史后验和技术指标驱动的智能候选池
- 服务端可配置五路 OpenAI 兼容模型，单路失败自动使用本地启发式
- AI 页面显示服务端 `.env` 加载路径和状态，并可复制不含真实密钥的配置模板
