...
- `backtest.rs`：无未来数据的逐日信号重放、交易成本和策略指标。
- `paper.rs`：模拟订单、成交、费用、T+1、估值、权益快照和持仓风险计划监控。
- `perf.rs`：信号未来收益回填及聚合。
...
SQLite 使用 WAL、外键和 5 秒 busy timeout。`AppState` 当前持有单连接互斥锁，适合单进程家庭部署。订单冻结、成交、撤单、研究风险计划、估值标记和快照均在事务中更新。持仓快照只有在全部股票取得有效行情后才提交。风险中心读取最新行情并计算提醒状态，不自动执行交易。
